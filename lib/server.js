...
function Server() {
  ...
  var defaultConfig = { helo: "O2HELO", domains: [] };
  ...
  server.listenTransport = function( type, ip, port, config ) { 
    ...
    for ( var param in defaultConfig ) {
      ...
      config[param] = JSON.parse(JSON.stringify(defaultConfig[param]));
      }
    }
    ...
    server.timeoutCheckingInterval = setInterval( function() {
      for( var i in server.connections ) {
        ...
        if ( server.connections[i].timeout < new Date().getTime() ) {
          server.connections[i].removeTransport();
          delete server.connections[i];
        }
      }
