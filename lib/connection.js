...
function Connection()
{
  ...
  connection._receiveBuffer = new Buffer(0);
...
Connection.prototype._translateData = function( dataReceived ) {
    ...
    self._receiveBuffer = Buffer.concat([self._receiveBuffer,data],self._receiveBuffer.length+data.length);
...
Connection.prototype._translateMessageCycle = function() {
    ...
    while (self._receiveBuffer.length > 0) {
        format = self._receiveBuffer.toString('utf8',0,1);
        switch(format) {
            case 'U':
                if (self._receiveBuffer.length < 5) return;
                messageLength = self._receiveBuffer.readUInt16LE(1);
                if (self._receiveBuffer.length < 5 + messageLength) return;
            ...
            case 'J':
                if (self._receiveBuffer.length < 5) return;
                messageLength = self._receiveBuffer.readUInt32LE(1);
                if (self._receiveBuffer.length < 5 + messageLength) return;
                length = messageLength + 5;
                break;
            case 'B':
                if (self._receiveBuffer.length < 3) return;
                typeLength = self._receiveBuffer.readUInt16LE(1);
                if (self._receiveBuffer.length < 3 + typeLength + 4) return;
                messageLength = self._receiveBuffer.readUInt32LE(3 + typeLength);
                if (self._receiveBuffer.length < 7 + typeLength + messageLength) return;
            ...
            case 'S':
                ...
                break;
            default:
                if (self._receiveBuffer.length < 1 + Connection.tinySize) return;
                length = 1 + Connection.tinySize;
                break;
        }
        message = self._receiveBuffer.slice( 0, length );
        self._receiveBuffer = self._receiveBuffer.slice( length );
